<x-admin-layout>
    <div class="mb-6">
        <a href="{{ route('admin.accounting.journal-entries.show', $journalEntry) }}" class="text-sm text-gray-500 hover:text-gray-700">← {{ $journalEntry->entry_number }}</a>
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Edit Journal Entry</h1>
        <p class="text-gray-500 mt-1">Editing draft entry &mdash; debits must equal credits</p>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('admin.accounting.journal-entries.update', $journalEntry) }}" id="jeForm">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Entry Date <span class="text-red-500">*</span></label>
                    <input type="date" name="entry_date" value="{{ old('entry_date', $journalEntry->entry_date?->toDateString()) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Reference</label>
                    <input type="text" name="reference" value="{{ old('reference', $journalEntry->reference) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono tracking-wide focus:outline-none focus:ring-2 focus:ring-red-500">
                </div>
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                    <textarea name="notes" rows="2" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500">{{ old('notes', $journalEntry->notes) }}</textarea>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <tbody id="linesBody">
                            @php
                                $existingLines = old('lines') ?? $journalEntry->lines->map(fn($l) => [
                            ...
                            <tr class="line-row border-t border-gray-100">
                                <td class="px-3 py-2">
                                    <div class="account-wrap relative">
                                        <input type="hidden" name="lines[{{ $i }}][account_id]" class="account-value" value="{{ $line['account_id'] ?? '' }}">
                                ...
                                <td class="px-3 py-2">
                                    <div class="partner-wrap relative">
                                        <input type="hidden" name="lines[{{ $i }}][partner]" class="partner-value" value="{{ $line['partner'] ?? '' }}">
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" name="lines[{{ $i }}][description]" value="{{ $line['description'] ?? '' }}" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-red-400">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" name="lines[{{ $i }}][debit]" value="{{ $line['debit'] ?? 0 }}" step="0.01" min="0" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-1 focus:ring-red-400 debit-input" oninput="updateTotals()">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" name="lines[{{ $i }}][credit]" value="{{ $line['credit'] ?? 0 }}" step="0.01" min="0" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-1 focus:ring-red-400 credit-input" oninput="updateTotals()">
                                </td>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.accounting.journal-entries.show', $journalEntry) }}" class="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50">Cancel</a>
