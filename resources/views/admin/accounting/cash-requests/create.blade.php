<x-admin-layout>
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500 mt-0.5">Request cash in advance. Each line item must have a category — VAT is computed automatically.</p>
    </div>
    ...
    <div class="max-w-4xl bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('admin.accounting.cash-requests.store') }}" enctype="multipart/form-data" id="crForm">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="purpose" value="{{ old('purpose') }}"
                           ...
                           placeholder="e.g. Field purchases — office supplies and fuel" required>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Attachment <span class="text-xs text-gray-400">(optional)</span></label>
            ...
            <div class="mb-6">
                <div class="flex items-center justify-between mb-2">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">VAT (18%) is auto-applied per item. Zero-rated categories (★) have no VAT. You can manually override to No VAT per item and provide a justification.</p>
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <tbody id="itemsBody">
                            @php $oldItems = old('items', [['description'=>'','expense_category_id'=>'','estimated_amount'=>'']]); @endphp
                            ...
                            <tr class="item-row border-t border-gray-100">
                                ...
                                <td class="px-3 py-2">
                                    <select name="items[{{ $i }}][expense_category_id]" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm cat-select" required>
                                    ...
                                    <div class="mt-1.5 flex items-center gap-2">
                                        <label class="flex items-center gap-1 text-xs text-gray-500 cursor-pointer select-none">
                                            <input type="checkbox" name="items[{{ $i }}][zero_vat_override]" value="1"
                                                   class="zero-override rounded"
                                                   {{ old("items.{$i}.zero_vat_override") ? 'checked' : '' }}>
                                            <span>No VAT</span>
                                        </label>
                                        <input type="text" name="items[{{ $i }}][vat_justification]"
                                               value="{{ old("items.{$i}.vat_justification") }}"
                                ...
                                <td class="px-3 py-2">
                                    <input type="number" name="items[{{ $i }}][estimated_amount]" value="{{ $item['estimated_amount'] ?? '' }}"
