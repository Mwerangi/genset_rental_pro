<x-admin-layout>
    ...
    <div class="max-w-3xl bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('admin.accounting.cash-requests.store') }}" id="crForm">
            ...
            <div class="space-y-4 mb-6">
                <div>
                    ...
                    <input type="text" name="purpose" value="{{ old('purpose') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="e.g. Field trip to Mombasa — fuel and allowances" required>
                </div>
                <div>
                    ...
                    <table class="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <tbody id="itemsBody">
                            ...
                            <tr class="item-row border-t border-gray-100">
                                <td class="px-3 py-2">
                                    <input type="text" name="items[{{ $i }}][description]" value="{{ $item['description'] ?? '' }}" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-red-400" placeholder="e.g. Diesel for Generator" required>
                                </td>
                                <td class="px-3 py-2 min-w-[200px]">
                                    <select name="items[{{ $i }}][expense_category_id]" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-red-400 cat-select" required>
    ...
    document.getElementById('addItem').addEventListener('click', () => {
        ...
        tr.innerHTML = `
        <td class="px-3 py-2"><input type="text" name="items[${itemIndex}][description]" class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm" placeholder="e.g. Diesel for Generator" required></td>
        <td class="px-3 py-2 min-w-[200px]">
