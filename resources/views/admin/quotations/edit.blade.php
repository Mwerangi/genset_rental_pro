    ...
    <div x-data="quotationBuilder()">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.quotations.show', $quotation) }}" class="text-slate-600 hover:text-slate-900 transition">
                ...
                <div>
                    ...
                    <p class="text-slate-600 mt-1">{{ $quotation->quotation_number }}</p>
                </div>
            </div>
                        ...
                        <x-card>
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium text-slate-900">{{ $quotation->quoteRequest->email }}</p>
                                </div>
                                <div>
                                    <p class="text-slate-600">Phone</p>
                                    <p class="font-medium text-slate-900">{{ $quotation->quoteRequest->phone }}</p>
                                </div>
                                <div>
                                    <p class="text-slate-600">Company</p>
                                    <p class="font-medium text-slate-900">{{ $quotation->quoteRequest->company_name ?? '-' }}</p>
                                </div>
                            </div>
                        </x-card>
                    @endif
                    ...
                    <x-card>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                >{{ old('payment_terms', $quotation->payment_terms) }}</x-textarea>
                            </div>
                            ...
                            <div>
                                ...
                                >{{ old('terms_conditions', $quotation->terms_conditions) }}</x-textarea>
                            </div>
                            ...
                            <div>
                                ...
                                >{{ old('notes', $quotation->notes) }}</x-textarea>
                            </div>
                        </div>
                    </x-card>
                </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="currency" value="TZS" x-model="currency" class="text-red-600" {{ old('currency', $quotation->currency ?? 'TZS') === 'TZS' ? 'checked' : '' }}>
                                    <span class="text-sm font-medium text-slate-700">TZS — Tanzanian Shilling</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="currency" value="USD" x-model="currency" class="text-red-600" {{ old('currency', $quotation->currency ?? 'TZS') === 'USD' ? 'checked' : '' }}>
                            ...
                            <div x-show="currency === 'USD'" x-cloak class="mt-3">
                                ...
                                <x-input type="number" name="exchange_rate_to_tzs" :value="old('exchange_rate_to_tzs', $quotation->exchange_rate_to_tzs)" step="0.0001" min="1" placeholder="e.g. 2650" />
                                <p class="text-xs text-slate-500 mt-1">Current locked rate on this quotation.</p>
                        ...
                        <div class="space-y-3">
                            @if($quotation->status === 'draft')
                                <button type="submit" name="send" class="w-full bg-red-600 text-white px-4 py-3 rounded-lg hover:bg-red-700 transition font-semibold shadow-lg shadow-red-600/20">
                                    Update & Send to Customer
                                </button>
                            @endif
                            <button type="submit" class="w-full bg-slate-600 text-white px-4 py-3 rounded-lg hover:bg-slate-700 transition font-semibold">
                                Update Quotation
                            </button>
                            <a href="{{ route('admin.quotations.show', $quotation) }}" class="block w-full text-center px-4 py-3 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 transition font-medium">
                                Cancel
                            </a>
                        </div>
                    </x-card>
    ...
    <script>
        ...
        function quotationBuilder() {
            return {
                ...
                isZeroRated: {{ old('is_zero_rated', $quotation->is_zero_rated) ? 'true' : 'false' }},
                totals: {
                    subtotal: 0,
                    vat: 0,
                    total: 0
                ...
                init() {
                    // Load existing items from quotation
